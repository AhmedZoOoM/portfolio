...
const phoneHeaderStyles = responsive.match(/@media\s*\(max-width:\s*620px\)\s*\{[\s\S]*$/)?.[0] || "";
assert.match(phoneHeaderStyles, /\.brand\s*\{[^}]*flex-basis:\s*100%/, "the brand must own the first phone header row");
assert.match(phoneHeaderStyles, /\.theme-toggle\s*\{[^}]*margin-left:\s*auto/, "the phone action row must align to the right from the theme toggle");
assert.match(phoneHeaderStyles, /\.nav-cta\s*\{[^}]*margin-left:\s*0/, "the CTA must remain grouped beside the theme toggle");
